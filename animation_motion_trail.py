...
bl_info = {
    "name": "Motion Trail",
    "author": "Bart Crouch, Phil Manzano - Realistis",
    "version": (3, 1, 4),
    "blender": (3, 6, 0),
...
def draw_callback(self, context):
    ...
    if context.window_manager.motion_trail.path_style == 'simple':
        ...
        for objectname, path in self.paths.items():
            points = []
            for x, y, c, frame, action_ob, child in path:
        ...
        for objectname, path in self.paths.items():
            for i, [x, y, color, frame, action_ob, child] in enumerate(path):
                ...
                if i != 0:
                    ...
                    points = [(int(halfway[0]), int(halfway[1])), (x, y)]
                    batch = batch_for_shader(shader, 'LINE_STRIP', {"pos": points})
                ...
                if i != len(path) - 1:
                    ...
                    points = [(x, y), (int(halfway[0]), int(halfway[1]))]
                    batch = batch_for_shader(shader, 'LINE_STRIP', {"pos": points})
                    shader.uniform_float("color", col)
                    batch.draw(shader)
        ...
        for objectname, path in self.paths.items():
            ...
            for x, y, color, frame, action_ob, child in path:
                ...
                and abs(frame - self.active_frame[1]) < 1e-4:
                    ...
                    points_white.append((x, y))
            if points_white:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_white})
                shader.uniform_float("color", white)
                batch.draw(shader)
            if points_orange:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_orange})
        ...
        for objectname, values in self.timebeads.items():
            ...
            for frame, coords in values.items():
                ...
                abs(frame - self.active_timebead[1]) < 1e-4:
                    points_orange.append((coords[0], coords[1]))
                else:
                    points_green.append((coords[0], coords[1]))
            if points_green:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_green})
                shader.uniform_float("color", green)
                batch.draw(shader)
            if points_orange:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_orange})
                shader.uniform_float("color", orange)
    ...
    if context.window_manager.motion_trail.mode == 'location':
        ...
        for objectname, values in self.handles.items():
            ...
            for frame, sides in values.items():
                if frame < limit_min or frame > limit_max:
                    continue
                kf_coords = self.keyframes[objectname][frame]
                for side, coords in sides.items():
                    ...
                    abs(frame - self.active_handle[1]) < 1e-4:
                        lines_brown.append((kf_coords[0], kf_coords[1]))
                        lines_brown.append((coords[0], coords[1]))
                    else:
                        lines_black.append((kf_coords[0], kf_coords[1]))
                        lines_black.append((coords[0], coords[1]))
            if lines_black:
                batch = batch_for_shader(shader, 'LINES', {"pos": lines_black})
                shader.uniform_float("color", black)
                batch.draw(shader)
            if lines_brown:
                batch = batch_for_shader(shader, 'LINES', {"pos": lines_brown})
                shader.uniform_float("color", brown)
                batch.draw(shader)
            ...
            for frame, sides in values.items():
                ...
                for side, coords in sides.items():
                    ...
                    abs(frame - self.active_handle[1]) < 1e-4:
                        ...
                        points_yellow.append((coords[0], coords[1]))
            if points_yellow:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_yellow})
                shader.uniform_float("color", yellow)
                batch.draw(shader)
            if points_orange:
                batch = batch_for_shader(shader, 'POINTS', {"pos": points_orange})
                shader.uniform_float("color", orange)
...
class MotionTrailOperator(bpy.types.Operator):
    ...
    _handle_calc = None
    _handle_draw = None
    _instance = None  # keep a reference to prevent garbage collection
    ...
    def handle_remove():
        ...
        MotionTrailOperator._instance = None
    ...
    def invoke(self, context, event):
        ...
        if not context.window_manager.motion_trail.enabled:
            ...
            MotionTrailOperator._instance = self  # prevent garbage collection
